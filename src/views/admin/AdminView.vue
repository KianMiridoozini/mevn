<template>
  <div class="bg-[#181818] min-h-screen lazy-css">
    ...
    <div v-else-if="error" class="text-center text-red-500"> </div>                         <!-- Error message -->
    ...
    <div class="my-8 p-2 w-full">
      ...
      <div class="mb-4 p-4 border rounded bg-[#181818]" v-for="product in products" :key="product._id"> <!-- Loop through the products -->
        ...
        <div class="mt-4 flex space-x-2"> <!-- Update and delete buttons -->
          <p>ID: {{ product._id }} </p> <!-- Product ID for testing -->
...
const {products, error, loading, getTokenAndUserId , fetchProducts, deleteProduct, addProduct, updateProduct} = useProducts();
...
const addProductHandler = async () => {
  const {userId} = getTokenAndUserId(); // Get user ID from the useProducts module
